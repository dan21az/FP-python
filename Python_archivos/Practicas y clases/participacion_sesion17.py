...
for linea in estudiantes:
    linea = linea.split(",")
    estado = linea[15]
    if estado not in diccionario_club:
        diccionario_club[estado] = 1
    else:
        diccionario_club[estado] += 1

for estado, contador in diccionario_club.items():
    porcentaje = (contador / sum(diccionario_club.values())) *100
    print(f"De {estado} hay {contador} estudiantes, que representan el {porcentaje}%")
