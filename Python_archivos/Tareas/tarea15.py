...
for linea in estudiantes:
    ...
    ciudad = linea[2]
    genero = linea[12]
    if ciudad not in dic_ciudad_genero:
        dic_ciudad_genero[ciudad] = [0,0] #[F,M]
    if genero == "F":
        dic_ciudad_genero[ciudad][0] +=1
    else:
        dic_ciudad_genero[ciudad][1] +=1

for ciudad, n_genero in dic_ciudad_genero.items():
    print(f"En {ciudad} hay {n_genero[0]} estudiantes femeninos y {n_genero[1]} estudiantes masculinos")
